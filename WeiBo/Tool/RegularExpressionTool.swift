...
extension NSRegularExpression {
    ...
    func enumerateSeparatedByRegex(in string: String, range: NSRange, using block: (Range<String.Index>, UnsafeMutablePointer<ObjCBool>) -> Swift.Void) {
        ...
        if rangeArray.count == 0 {
            tempRange = NSRange(location: 0, length: string.count)
            block(Range(tempRange, in: string)!, &stop)
            return
        }
        ...
        if let lastRange = rangeArray.last {
            if lastRange.upperBound < string.count {
                let nsRange = NSRange(location: lastRange.upperBound, length: string.count - lastRange.upperBound)
                block(Range(nsRange, in: string)!, &stop)
            }
        }
    }
}
